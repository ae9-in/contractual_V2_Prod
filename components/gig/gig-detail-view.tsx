...
export function GigDetailView({ gig }: { gig: MockGigDetail }) {
  ...
  const mainSrc = gig.gallery[activeThumb] ?? gig.gallery[0]
  const isVideoThumb = activeThumb === Math.min(3, gig.gallery.length - 1)
                  ...
                  {gig.gallery.map((src, idx) => (
                    ...
                    >
                      <Image
                        ...
                        sizes="80px"
                      />
                      {idx === Math.min(3, gig.gallery.length - 1) && (
