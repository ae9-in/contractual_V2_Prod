...
function formatBudget(
  ...
  if (min !== max && min != null && max != null) {
    return {
      label: `${sym}${Math.round(min).toLocaleString("en-IN")} – ${sym}${Math.round(max).toLocaleString("en-IN")}${suffix}`,
      min,
      max,
    }
  }

  const amt = Math.round(budgetAmount || min || 0)
  return {
    label: `${sym}${amt.toLocaleString("en-IN")}${suffix}`,
    min: amt,
    max: amt,
  }
}
