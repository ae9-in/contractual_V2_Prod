...
export function ContractDetailPage({ contractId }: { contractId: string }) {
  ...
  const { data: contract, isLoading, isError } = useQuery({
    ...
    // We can rely on sockets, but a fallback is good
  ...
  useEffect(() => {
    ...
    socket.on(SOCKET_EVENTS.CONTRACT_STATUS, onUpdate)
    socket.on(SOCKET_EVENTS.SUBMISSION_NEW, onUpdate)
    socket.on(SOCKET_EVENTS.MESSAGE_NEW, onUpdate)

    return () => {
      socket.emit("leave:contract", contractId)
      socket.off(SOCKET_EVENTS.CONTRACT_STATUS, onUpdate)
      socket.off(SOCKET_EVENTS.SUBMISSION_NEW, onUpdate)
      socket.off(SOCKET_EVENTS.MESSAGE_NEW, onUpdate)
    }
  }, [socket, contractId, qc])
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(135deg, #f0f4ff 0%, #fafbff 25%, #f0fdf9 50%, #fdf4ff 75%, #fff7ed 100%)" }}>
          ...
          <div className="relative z-10">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="shrink-0 text-right">
                      <p className="text-[11px] uppercase tracking-wider text-teal-300/50 font-semibold">Contract Range / Value</p>
                      {c.gig?.minBudget && c.gig?.maxBudget ? (
                        <p className="mt-1 text-2xl sm:text-3xl font-black tracking-tight">
                          {formatCurrency(c.gig.minBudget)} — {formatCurrency(c.gig.maxBudget)}
                        </p>
                      ) : (
      ...
      <Dialog open={showSubmitDialog} onOpenChange={setShowSubmitDialog}>
        <DialogContent className="sm:max-w-2xl max-h-[95vh] overflow-y-auto rounded-3xl p-0 border-none shadow-2xl">
          <div className="p-8">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <label className="text-sm font-bold text-gray-900">Project Files</label>
                <span className="text-[10px] font-black uppercase tracking-widest text-[#6d9c9f]">Max 30MB · Multi-select</span>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <UploadButton
                  ...
                  appearance={{
                    button: "flex-row-reverse gap-2 bg-white border-2 border-dashed border-gray-200 text-gray-500 hover:border-teal-500 hover:bg-teal-50/30 transition-all h-14 w-full rounded-2xl text-sm font-bold shadow-none after:bg-teal-600",
                    container: "w-full",
                    allowedContent: "hidden"
                  }}
                  content={{
                    button: "Choose files or drag & drop"
                  }}
                />
              </div>
            ...
            <DialogFooter className="mt-4 flex flex-col sm:flex-row gap-3">
              <Button variant="outline" onClick={() => setShowSubmitDialog(false)} className="rounded-xl px-6 h-11 font-bold text-gray-500 border-gray-200 order-2 sm:order-1">
                Cancel
              </Button>
              <Button
                ...
                className="rounded-xl bg-gradient-to-r from-teal-600 to-teal-700 px-10 h-11 font-black text-white shadow-xl shadow-teal-500/20 hover:shadow-teal-500/40 transition-all order-1 sm:order-2"
              >
