  ...
  return (
    ...
    >
      <View style={{ position: 'relative' }}>
        <View style={{ width: 52, height: 52, borderRadius: 16, backgroundColor: '#e2e8f0',
        ...
        {item.isOnline && (
          <View style={{ position: 'absolute', bottom: 1, right: 1, width: 12, height: 12,
            borderRadius: 6, backgroundColor: '#22c55e', borderWidth: 2, borderColor: 'white' }} />
        )}
        {item.unreadCount > 0 && (
          <View style={{ position: 'absolute', top: -4, right: -4, minWidth: 20, height: 20,
      ...
      <View style={{ flex: 1 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
          <Text style={{ fontSize: 15, fontWeight: item.unreadCount > 0 ? '900' : '700',
            color: '#1c2b2c', flex: 1, marginRight: 8 }} numberOfLines={1}>
            {item.recipientName}
          </Text>
        ...
        <Text style={{ fontSize: 13, color: item.unreadCount > 0 ? '#374151' : '#94a3b8',
          fontWeight: item.unreadCount > 0 ? '600' : '500' }} numberOfLines={1}>
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <View>
            ...
            {totalUnread > 0 && (
              <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13, fontWeight: '600', marginTop: 2 }}>
                {totalUnread} unread
              </Text>
            )}
          </View>
          <TouchableOpacity style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 14, padding: 10 }}>
            <Filter size={20} color="white" />
          </TouchableOpacity>
        </View>

        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.1)',
          ...
          <TextInput
            ...
            style={{ flex: 1, color: 'white', fontWeight: '600', fontSize: 14 }}
          />
        </View>
      </LinearGradient>
          ...
          ListEmptyComponent={
            <View style={{ alignItems: 'center', paddingVertical: 60, paddingHorizontal: 30 }}>
              ...
              <Text style={{ color: '#62737a', textAlign: 'center', lineHeight: 22, fontSize: 14 }}>
                {search ? `No conversations match "${search}"` : 'Your conversations with clients will appear here.'}
              </Text>
            </View>
          }
        />
